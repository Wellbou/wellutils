...
Пятнадцать файлов-инструментов, которые рассказывают, что делает
ваша машина: USB и PCI-устройства, накопители, память, топология
процессора, графика, модули ядра, температуры, периферия, сеть,
питание и агрегатор здоровья. У каждого инструмента один и тот же
CLI - общие флаги, общие коды возврата, общий вывод в рамке.
Лаунчер (`wellutils`) связывает их вместе, а короткие алиасы
(`wusb`, `wpci`, `wmem`, ...) устанавливаются рядом.
...
`wpower`/`wbatt`, `wdoc`/`wdoctor`. Лаунчер принимает и их:
...
В комплекте есть **SAMPLES.md** - реальный вывод всех утилит с машины
автора: можно заранее посмотреть, что и как печатает каждая команда:
