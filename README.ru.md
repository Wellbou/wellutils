...
<div align="center">
  ...
  [![Версия: 1.4.0-55](https://img.shields.io/badge/Version-1.4.0--55-22272E)](PKGBUILD)
...
- **Без root:** `./install.sh --prefix=$HOME/.local` ставит всё в
  ...
  в `PATH`. Если префикс недоступен для записи (по умолчанию
  `/usr/local`), установщик сначала сообщает, что будет использовать
  `sudo`/`doas`; без терминала он продолжает, только если `sudo -n`
  работает без пароля.
- **Файлы пакетов:** установщик не перезаписывает файлы, принадлежащие
  пакету дистрибутива (проверка через `pacman -Qo` / `dpkg -S` /
  `rpm -qf`), например AUR-сборку wellutils в том же префиксе: такой
  файл пропускается, код выхода 2. `--force` перезаписывает всё равно.
  `--uninstall` удаляет только записи манифеста внутри префикса.
- **Коды выхода установщика:** 0 ok, 1 проблема окружения (нет bash 4,
  нет прав, нет curl/wget), 2 неверная опция или часть файлов
  пропущена/не записана.
...
флаг `--yes` пропускает вопрос (для скриптов и cron). На Arch
`--check` использует `checkupdates` (pacman-contrib), если он есть, —
список свежий без синхронизации настоящей базы. На Debian/Ubuntu
выполняется `apt-get upgrade` (пакеты не удаляются); `--full`
включает `apt-get full-upgrade`. Коды выхода: 0 ok, 2 ошибка CLI,
3 нет пакетного менеджера / ошибка обновления или самообновления (с
`--json` при сбое получения версии всё равно печатается объект
`"status": "no-remote"`, затем выход с 3). Умеет он и
...
wellup --full                 # apt: full-upgrade (может удалять пакеты)
...
Второй круг аудита: я перепроверил все находки из прошлого раза и
прогнал свежий взгляд по новому коду - и там нашлось немало настоящих
багов. Большинство кусается только на необычном железе, так что
неудивительно, что они так долго жили.

- Падения из-за `set -e`: несколько функций могли вернуть 1 последней
  строкой (нет узла в sysfs, `nvidia-smi` пишет `N/A`) и убить весь тул.
  Теперь они заканчиваются на `return 0`, и у каждого `$(...)`, который
  может упасть, есть запасной вариант.
- Число ядер/потоков на гибридных CPU (Alder Lake, RK3399, Apple M1)
  считалось неверно: читалась только первая секция lscpu. Потоки теперь
  берутся из настоящего числа CPU, ядра - из уникальных пар
  physical/core id.
- `wellsensors` прятал сенсоры ровно когда они горячие (флаг alarm
  выкидывал их из отчёта) и не показывал вентиляторы на Asahi/POWER.
  И то, и другое исправлено.
- Классификация USB переписана: Bluetooth-адаптеры, RNDIS-гаджеты,
  геймпады Xbox, MTP-телефоны и HID-клавиатуры/мыши определяются по
  кодам классов и драйверам, а не угадыванием по имени.
- Инъекция escape-последовательностей: враждебное имя USB/DMI/диска
  могло писать escape-коды в терминал. Управляющие символы теперь
  вырезаются; битый UTF-8 больше не ломает `--json`.
- `wellnet` переживает падающий `ip` (Termux/Android) и уходит на
  `/proc`, когда утилиты есть, но не работают.
- `wellfetch` определяет настоящий терминал (kitty/WezTerm/Konsole/...),
  правильно считает пакеты, CPU и устройства ввода.
- `welldoctor` вообще обходится без временных файлов и читает больше
  чипов сенсоров; `wellup` выучил Clear Linux (`swupd`), разбор версий
  Void и `--full` для apt.
- Безопасность установщика: `--uninstall` не трогает файлы вне префикса,
  а установка поверх файлов дистрибутива теперь отказывается без
  `--force`.
- Узкие терминалы: длинные подсказки переносятся, а не вылезают за 60
  колонок.
